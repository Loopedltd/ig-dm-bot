 ...
 *
 * The AI demo (/api/demo/chat) is shared with trial/routes.js — no duplication needed.
 * This file is a deliberate copy of the trial page template, not a shared module.
 */
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .reel-wrap { display: flex; justify-content: center; margin-left: 105px; margin-top: 62px; }
    ...
    .fc-icon { width: 44px; height: 44px; border-radius: 13px; background: rgba(45,107,255,0.08); border: 1px solid rgba(45,107,255,0.16); display: flex; align-items: center; justify-content: center; position: absolute; top: 15px; left: 15px; }
    .fc-title { font-size: 14px; font-weight: 800; color: #0f172a; margin-bottom: 4px; line-height: 1.3; }
    .fc-desc { font-size: 11px; color: rgba(15,23,42,0.55); line-height: 1.5; }
    /* Card 1 — Calendar */
    .fc-cal-inner { display: flex; flex-direction: column; align-items: center; gap: 2px; opacity: 0; }
    .fc-cal-grid { display: grid; grid-template-columns: repeat(3,9px); gap: 2px; }
    .fc-cal-cell { width: 9px; height: 9px; border-radius: 2px; background: rgba(15,23,42,0.07); }
    .fc-cal-pick { transition: background 0.3s, box-shadow 0.3s; }
    .fc-cal-confirm { font-size: 11px; font-weight: 700; color: #0f172a; opacity: 0; }
    ...
    .fc-clock { width: 26px; height: 26px; border-radius: 50%; border: 1.5px solid rgba(15,23,42,0.22); position: relative; flex-shrink: 0; }
    ...
    .fc-ch-hr { width:1.5px; height:8px; margin-left:-0.75px; }
    .fc-ch-mn { width:1px; height:11px; margin-left:-0.5px; }
    ...
    .fc-clock-bubble { width:13px; height:9px; border-radius:3px 3px 3px 0; background:rgba(45,107,255,0.75); opacity:0; flex-shrink:0; }
    ...
    .fc-funnel { position: relative; width: 36px; height: 30px; flex-shrink: 0; }
    ...
    .fc-fd2 { left:26px; }
    .fc-fd3 { left:14px; }
    .fc-fd4 { left:14px; background:rgba(45,107,255,0.9); }
    ...
    @keyframes fcFOut { 0%,8%{opacity:0;transform:translateY(0)} 16%{opacity:0.8;transform:translateY(0)} 42%{opacity:0;transform:translateY(15px)} 100%{opacity:0;transform:translateY(15px)} }
    @keyframes fcFThru { 0%,8%{opacity:0;transform:translateY(0)} 16%{opacity:1;transform:translateY(0)} 50%{opacity:1;transform:translateY(26px)} 58%,100%{opacity:0;transform:translateY(26px)} }
    ...
    /* Card 4 — Dashboard */
    .fc-dash-bars { display: flex; align-items: flex-end; gap: 4px; height: 25px; opacity: 0; }
    .fc-dash-b { border-radius: 2px 2px 0 0; background: rgba(45,107,255,0.40); transform-origin: bottom; transform: scaleY(0); }
    .fc-db1 { width: 8px; height: 11px; }
    .fc-db2 { width: 8px; height: 20px; }
    .fc-db3 { width: 8px; height: 15px; }
    ...
    @media (max-width: 960px) { .feat-layout { flex-direction: column-reverse; align-items: center; } .feat-left .section-heading { text-align: center; } .reel-wrap { margin-left: 0; } .feat-cards { grid-template-columns: 1fr; grid-template-rows: auto; } .feat-card { width: 100%; height: 160px; } }
        ...
        <div class="feat-card" id="fc3">
          <div class="fc-icon">
            <div class="fc-funnel-wrap">
              <div class="fc-funnel">
                <svg class="fc-funnel-svg" viewBox="0 0 36 32" width="36" height="30" fill="none"><path d="M2 2L34 2L24 22L24 30L12 30L12 22Z" stroke="rgba(15,23,42,0.25)" stroke-width="1.5" stroke-linejoin="round" fill="rgba(15,23,42,0.04)"/></svg>
...
(function () {
  ...
  try {
    ...
    if (reelScroller && reelSegs.length) {
      ...
      var reelCard = document.querySelector('.reel-card');
      if (reelCard) reelCard.classList.add('reel-on-light'); // slide 1 is active on load
      ...
      function updateReelProgress() {
        ...
        if (reelCard) reelCard.classList.toggle('reel-on-light', idx === 0);
...
<script>
  async function startTrial(e) {
    ...
    try {
      ...
      const res = await fetch('/api/homepage/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
...
// ─────────────────────────────────────────────────────────────────────────────
// CHECKOUT — POST /api/homepage/checkout
// Creates a new client + payment_links row, then creates a Stripe session.
// ─────────────────────────────────────────────────────────────────────────────
router.post("/api/homepage/checkout", async (req, res) => {
  try {
    if (!stripe) {
      return res.status(500).json({ error: "Stripe not configured on this server" });
    }

    // 1. Create a new client record (same pattern as ig_signup new-coach flow)
    const { data: newClient, error: clientErr } = await supabase
      .from("clients")
      .insert({ name: "New Coach", timezone: "Europe/London" })
      .select()
      .single();

    if (clientErr || !newClient) {
      console.error("[home/checkout] client insert failed:", clientErr?.message);
      return res.status(500).json({ error: "Failed to create account. Please try again." });
    }

    const clientId = newClient.id;

    // 2. Create a default client_configs row (same defaults as ig_signup)
    const { error: configErr } = await supabase.from("client_configs").insert({
      ...
      system_prompt:
        ...
        "Keep replies short, casual and conversational. Ask one question at a time to understand the " +
        "lead's goals and situation before moving towards booking a call.",
      tone: "direct",
      style: "short, punchy",
    ...
    if (configErr) {
      console.error("[home/checkout] client_configs insert failed:", configErr?.message);
      await supabase.from("clients").delete().eq("id", clientId);
      return res.status(500).json({ error: "Failed to initialise account. Please try again." });
    }

    // 3. Create setup token in payment_links so /set-password works as normal
    ...
    if (plErr) {
      console.error("[home/checkout] payment_links insert failed:", plErr?.message);
      await supabase.from("client_configs").delete().eq("client_id", clientId);
      await supabase.from("clients").delete().eq("id", clientId);
      return res.status(500).json({ error: "Failed to prepare onboarding. Please try again." });
    }

    // 4. Create Stripe Checkout Session — fixed £100/month
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: "7-day free trial, then £100/month after trial",
            },
            unit_amount: HOME_MONTHLY_PENCE,
    ...
    console.log("[home/checkout] Stripe session created", { clientId, sessionId: session.id });
    return res.json({ url: session.url });
  } catch (e) {
    console.error("[home/checkout] error:", e?.message || e);
    return res.status(500).json({ error: "Checkout failed — please try again" });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// SUCCESS — GET /home/success
// Verifies the Stripe session and redirects to the shared /set-password flow.
// ─────────────────────────────────────────────────────────────────────────────
router.get("/home/success", async (req, res) => {
  const { session_id } = req.query;
  if (!session_id) return res.status(400).send("Missing session_id");

  if (!stripe) {
    console.error("[home/success] Stripe not configured");
  ...
  try {
    ...
    if (session.status !== "complete") {
      console.warn("[home/success] session not complete:", session.status, session_id);
      return res.status(400).send("Payment not completed.");
    }

    const setupToken = session.metadata?.payment_token;
    if (!setupToken) {
      console.error("[home/success] no payment_token in metadata", session_id);
    ...
    console.log("[home/success] verified, redirecting to set-password", { session_id });
    return res.redirect(302, `/set-password?token=${encodeURIComponent(setupToken)}`);
  } catch (e) {
    console.error("[home/success] error:", e?.message || e);
